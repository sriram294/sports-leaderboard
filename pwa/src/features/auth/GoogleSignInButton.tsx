...
function loadGoogleIdentityServices(): Promise<GoogleIdentityServices> {
  ...
  return new Promise((resolve, reject) => {
    script.addEventListener('load', () => {
      ...
      google ? resolve(google) : reject(new Error('Google Identity Services loaded without its API.'));
    }, { once: true });
    script.addEventListener('error', () => reject(new Error('Google Identity Services did not load.')), { once: true });
  });
}

/** Reusable Google authentication control for Login and the public deletion route. */
    ...
    loadGoogleIdentityServices().then(googleApi => {
      ...
      googleApi.accounts.id.initialize({
        ...
        callback: async response => {
          ...
          try {
            login(await api.googleSignIn(response.credential));
          } catch (cause) {
      ...
      if (buttonHost.current) {
        googleApi.accounts.id.renderButton(buttonHost.current, {
          ...
          shape: 'pill', width: 360, logo_alignment: 'center',
        });
      }
    }).catch(() => {
    ...
    <>
      {error && (
        <div className="form-error" role="alert">
          ...
          {error.code && <code className="error-code">Error code: {error.code}</code>}
        </div>
      )}
