...
| IOS-DEC-012 | 2026-09-01 | Match creation accepts an optional caller/group-scoped `Idempotency-Key`; iOS retains the exact body and key across retry until the form changes. | Prevents a lost response or duplicate tap from recording a second match and applying stats twice while preserving compatibility for existing clients that omit the header. |
| IOS-DEC-013 | 2026-09-15 | Team-v2 Skill rating remains server-owned and cumulative through the evaluation cutoff; iOS uses the selected range only for statistics, qualification, and form. | Keeps Android, iOS, and PWA rankings identical while avoiding divergent client-side Gaussian implementations. Optional version fields remain tolerant of the Wilson/team-v1 rollback path. |
